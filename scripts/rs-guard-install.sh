...
curl -fsSL -o "$ASSET_PATH" "${BASE_URL}/${RS_GUARD_ASSET}"
curl -fsSL -o "${ASSET_PATH}.sha256" "${BASE_URL}/${RS_GUARD_ASSET}.sha256"
...
if [[ "$ACTUAL_SHA256" != "$RS_GUARD_SHA256" ]]; then
  ...
  echo "Expected: $RS_GUARD_SHA256" >&2
...
if [[ "$(uname -s)" == "Linux" ]]; then
  ...
  echo "Checksum verified for $OUTPUT_PATH (binary execution skipped on non-Linux host)."
fi
